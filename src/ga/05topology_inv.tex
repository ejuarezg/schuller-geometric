...
\be
The interval $[0,1]$ is compact in $(\R,\cO_\mathrm{std})$ since it is clearly closed and bounded. Hence, it is compact by the Heine-Borel theorem.
\ee
...
\bse
\mathscr{L}_p := \{\g\cl[0,1]\to M \mid \g \t{ is continuous and } \g(0)=\g(1)=p\}.
\ese
\ed
...
\bse
(\g * \delta) (\l):= \left\{ \ba{ll} \g(2\l) & \t{if } 0\leq \l < \tfrac{1}{2}\\ \delta(2\l-1) & \t{if } \tfrac{1}{2}\leq \l \leq 1 \ea \right.
\ese
\ed

\bd
Let $(M,\cO)$ be a topological space. The \emph{fundamental group}\index{fundamental group} $(\pi_1(p),\bullet)$\index{$\pi_1(p)$} of $(M,\cO)$ at $p\in M$ is the set:
\bse
...
\br
A group\index{group} is a pair $(G,\bullet)$ where $G$ is a set and $\bullet \cl G\times G \to G$ is a map (also called \emph{binary operation}) such that:
\ben
