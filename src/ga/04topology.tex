...
\een
This is called the \emph{chaotic topology}\index{topology!chaotic} (or
\emph{indiscrete topology}) and can be defined on any set.
\ee
...
Then we have:
\bse
\cO_{M/\!\sim} := \{U \se M/\!\sim \ \mid \mathrm{preim}_q(U) \in \cO \}.
\ese
...
\ee
\bt[Convergence in $\R^d$]\index{convergence!in $\R^d$}
